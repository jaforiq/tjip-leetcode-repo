...
public:
    ...
    int BASE = 1000000007;
    long long getSerialValue(int x, int y){
        return (1LL *x * BASE) + y;
    }
    DetectSquares() {
        points.clear();
    ...
    int count(vector<int> qpoint) {
        ...
        for(auto point : points){
            ...
            if(points.count(getSerialValue(x4, y2)) && points.count(getSerialValue(x2, y4))){
                total += points[getSerialValue(x4, y2)] * points[getSerialValue(x2, y4)] * point.second;
            }
        }
        return total;
    }
